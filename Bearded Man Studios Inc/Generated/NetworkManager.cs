...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] ShipMoveNetworkObject = null;
		public GameObject[] ShootBulletNetworkObject = null;
		public GameObject[] SpaceMoveNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is ShipMoveNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (ShipMoveNetworkObject.Length > 0 && ShipMoveNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(ShipMoveNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<ShipMoveBehavior>();
						}
					}
			...
			else if (obj is ShootBulletNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (ShootBulletNetworkObject.Length > 0 && ShootBulletNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(ShootBulletNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<ShootBulletBehavior>();
						}
					}
			...
			else if (obj is SpaceMoveNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (SpaceMoveNetworkObject.Length > 0 && SpaceMoveNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(SpaceMoveNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<SpaceMoveBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateShipMove instead, its shorter and easier to type out ;)")]
		public ShipMoveBehavior InstantiateShipMoveNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ShipMoveNetworkObject[index]);
			var netBehavior = go.GetComponent<ShipMoveBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<ShipMoveBehavior>().networkObject = (ShipMoveNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateShootBullet instead, its shorter and easier to type out ;)")]
		public ShootBulletBehavior InstantiateShootBulletNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ShootBulletNetworkObject[index]);
			var netBehavior = go.GetComponent<ShootBulletBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<ShootBulletBehavior>().networkObject = (ShootBulletNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateSpaceMove instead, its shorter and easier to type out ;)")]
		public SpaceMoveBehavior InstantiateSpaceMoveNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SpaceMoveNetworkObject[index]);
			var netBehavior = go.GetComponent<SpaceMoveBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<SpaceMoveBehavior>().networkObject = (SpaceMoveNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of ShipMove
		/// </summary>
		/// <returns>
		/// A local instance of ShipMoveBehavior
		/// </returns>
		/// <param name="index">The index of the ShipMove prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public ShipMoveBehavior InstantiateShipMove(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (ShipMoveNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: ShipMove. Add them at the NetworkManager prefab.");
				return null;
			}
		...
		/// <summary>
		/// Instantiate an instance of ShootBullet
		/// </summary>
		/// <returns>
		/// A local instance of ShootBulletBehavior
		/// </returns>
		/// <param name="index">The index of the ShootBullet prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public ShootBulletBehavior InstantiateShootBullet(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (ShootBulletNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: ShootBullet. Add them at the NetworkManager prefab.");
				return null;
			}
		...
		/// <summary>
		/// Instantiate an instance of SpaceMove
		/// </summary>
		/// <returns>
		/// A local instance of SpaceMoveBehavior
		/// </returns>
		/// <param name="index">The index of the SpaceMove prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public SpaceMoveBehavior InstantiateSpaceMove(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (SpaceMoveNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: SpaceMove. Add them at the NetworkManager prefab.");
				return null;
			}
